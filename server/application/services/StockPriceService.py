...
from application.schemas.StockPriceSchema import stock_price_schema, stock_price_list_schema, stock_price_paging_schema
from sqlalchemy import desc, asc
from application.technical_analysis.indicators.momentum import rsi, awesome_oscillator, stoch_d, stoch_k, williams_r
from application.technical_analysis.indicators.volatility import average_true_range, bollinger_hband_indicator, \
    bollinger_lband_indicator, keltner_channel_hband_indicator, keltner_channel_lband_indicator
from application.technical_analysis.indicators.volume import chaikin_money_flow, money_flow_index, on_balance_volume
from application.technical_analysis.indicators.trend import ema, sma, macd, macd_signal, adx, cci, aroon_down, aroon_up, \
    psar_up_indicator, psar_down_indicator
from application.technical_analysis.candlestick.candlestick import single_candle, double_candles, triple_candles
import pandas as pd
import math
...
def calculate_by_indicator(indicator, df):
    ...
    #     return ema(close=df['close'], window=20)
        ...
        return keltner_channel_hband_indicator(high=df['high'], low=df['low'], close=df['close'], window=20,
                                               window_atr=10)
    elif indicator == 'keltner20_channel_lband_indicator':
        return keltner_channel_lband_indicator(high=df['high'], low=df['low'], close=df['close'], window=20,
                                               window_atr=10)
...
def calculate_indicator_by_symbol_and_date(indicators, symbol, date):
    ...
    data = StockPrice.query.filter(StockPrice.symbol == symbol, StockPrice.stock_date >= from_date).order_by(
        asc('stock_date'))
    df = get_df_stock_price_data(data)
    for indicator in indicators:
        df[indicator] = calculate_by_indicator(indicator, df)
